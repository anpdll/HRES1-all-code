...
Pel_min = 0.1;
Pel_max = 1;
yearly_demand_all = [562189, 396186, 222195, 171683, 83916]; %glass, fd, ceramics, metal, chemical
% mh2Year_required = 1.*yearly_demand;
hours_yearly_all = [6000, 6205, 6552, 6000, 6498];
mh2hour_limit_all = yearly_demand_all./hours_yearly_all;

% Economic Data
pv_per_kw = 981; % cost of panel per kW
battery_per_kwh = 276; % cost of battery per kWh
el_per_kw =  705;
% burner_conversion_cost = 0.07111603 * 1e+06; %chemical
...
tic
solution_combo = cell(5,10);

for i = 1:5
    yearly_demand_individual = yearly_demand_all(i);
    mh2hour_limit = mh2hour_limit_all(i);
    for j = 1:10
        mh2Year_required = (j/10).*yearly_demand_individual;
        ...
        problem.Objective = (1.35.*(el_per_kw.*Pel_rated2.*1000) + (pv_per_kw.*Ppv_rated2.*1000) + 1.1.*(battery_per_kwh.*ES_rated2.*1000)) ...
        ...
        problem.Constraints.constraint13 = mh22 == (nel.*P_el2.*1000)./hhv;
        problem.Constraints.constraint14 = sum(mh22) >= mh2Year_required;
        problem.Constraints.constraint15 = mh22 <= mh2hour_limit;
