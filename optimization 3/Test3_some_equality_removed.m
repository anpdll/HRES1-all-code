...
ES = optimvar('ES',nPeriods,1,'Type','continuous','LowerBound',0,'UpperBound',ES_rated_max);
...
mh2 = optimvar('mh2',nPeriods,1,'LowerBound',0);
...
Ppveq1 = P_pv == f_pv.*(Ppv_rated.*1e6).*(GHI./(G_std.*1000)).*(1 + alpha_pv.*(T_cell-T_cell_std))/(1e6); % MW
...
% Battery
Battery_capex = battery_per_kwh.*ES_rated;
...
min_lcoh.Constraints.Ppveq1 = Ppveq1;
min_lcoh.Constraints.mh2eq = mh2eq;
